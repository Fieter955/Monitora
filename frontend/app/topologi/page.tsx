...
export default function TopologyPage() {
  ...
  const load = useCallback(async () => {
    const [topologyData, deviceData, me] = await Promise.all([
      api<Topology>("/topology"), api<Device[]>("/devices"), api<User>("/auth/me"),
    ]);
  ...
  async function sync() {
    ...
    try { const result = await api<Topology>("/network/sync", {method: "POST"}); setTopology(result); setPositions(layoutNodes(result.nodes)); setMessage("Discovery port dan jalur koneksi selesai diperbarui."); }
    ...
    <AppShell>
      ...
      <div className="topology-layout">
        <section className="panel topology-panel">
          <div className="panel-header"><div><h2>Topologi aktif</h2><p>{topology?.nodes.length ?? 0} perangkat · {topology?.edges.length ?? 0} koneksi</p></div><div className="zoom-controls" aria-label="Kontrol zoom"><button onClick={() => setZoom((value) => Math.max(.65, value - .15))} aria-label="Perkecil">−</button><span>{Math.round(zoom * 100)}%</span><button onClick={() => setZoom((value) => Math.min(1.6, value + .15))} aria-label="Perbesar">+</button></div></div>
...
function layoutNodes(nodes: TopologyNode[]) { const groups = [nodes.filter((n)=>n.kind==="router"),nodes.filter((n)=>n.kind==="switch"||n.kind==="hub"),nodes.filter((n)=>!["router","switch","hub"].includes(n.kind))]; const positions: Record<number,Point>={}; groups.forEach((group,row)=>group.forEach((node,index)=>{positions[node.id]={x:70+index*(780/Math.max(1,group.length-1)),y:55+row*210};})); return positions; }
